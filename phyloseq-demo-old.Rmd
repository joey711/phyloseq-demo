...
```{r}
plot_richness_estimates(GP, "SampleType")
```

Probably easier to compare different classes of environments if we use color.
While we are at it, let's group by the new `"human"` variable, and add an extra layer with the boxplot geometric object, using the ggplot2 function `geom_boxplot`. 
...
(p  <- plot_network(ig, enterotype, color="SeqTech",
...
`r round(100*(GloPa.pcoa$values$Relative_eig[4])) ` \%, 
...
p <- ggplot(mdf, aes(Phylum, value, color=Phylum)) + geom_boxplot() +
  facet_wrap(~variable, 2) + scale_colour_hue(guide="none") +
