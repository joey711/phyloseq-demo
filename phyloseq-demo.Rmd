...
## More complicated import example: Bushman et al., combined enterotypes dataset
...
The "genus and phylum" abundance tables are provided online (ftp). The `import_direct` variable in this example determines whether to look locally or at the ftp for the files
```{r}
import_direct <- FALSE
```
Now try importing.
...
unlink(tmpdir)
  ...
  # import locally:
  biom_file <- "~/Downloads/study_1011_split_library_seqs_and_mapping/study_1011_closed_reference_otu_table.biom"
  map_file <- "~/Downloads/study_1011_split_library_seqs_and_mapping/study_1011_mapping_file.txt"
  # Now import the .biom-formatted otuTable-taxonomyTable file.
  biom_otu_tax <- import_biom(biom_file, "greengenes")
  # Add sample data to the dataset using merge
  bmsd <- import_qiime_sampleData(map_file)
...
Now the two data objects, containing the otuTabe-taxonomyTable and sampleData components, respectively. Note that if we had three objects that were all components (think single tables, or a tree), then we would use the constructor function, `phyloseq`. Because the `.biom` file contained two tables (including an otuTable), the `import_biom` function returned a `"phyloseq-class"` instance. Whenever adding/merging data for one (or more) phyloseq-class objects, use the merging function, `merge_phyloseq`, rather than the constructor (`phyloseq`).
```{r}
...
## Subsection: Abundance Accessors
...
Note how a *sample name* is required by `getSpecies`, and vice versa. 
...
Start by subsetting just the *Crenarchaeota* from all samples:
```{r}
...
of each sample (the total number of reads sequenced from a particular sample).
Thus we segway to transformations (e.g. convert to fractional abundance prior to filtering)
...
For sake of time of calculation/rendering in this tutorial, let's subset the data further to most abundant 5 phyla
```{r}
...
Using `GP100` from the previous section, let's calculate the unweighted-UniFrac distance for each sample pair in the dataset, and then perform Multidimensional Scaling (aka Principle Coordinates Analysis) on the resulting distance.
```{r}
...
# Section: Getting phyloseq data into other R tools
