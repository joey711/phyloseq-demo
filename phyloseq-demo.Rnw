...
\title{ Demo: phyloseq -- A Bioconductor package for handling and analysis of high-throughput phylogenetic sequence data }
...
\section{Summary}
This is a demonstration manual for the phyloseq package. It is a Sweave source-file with example code-chunks that can be reused if you have phyloseq installed.
...
Vignettes are included in phyloseq. A quick way to load them is:
...
This demonstration is to support a live demonstration of tools in phyloseq, and is a documentation supplement to the other resources available (e.g. wiki, vignettes, publications, function-level documentation, etc.).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{phyloseq_analysis-plot_tree_example.pdf}
